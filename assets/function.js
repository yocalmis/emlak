...
function contentGenerator(link, parent, isSpecific, identificator) {
	let flags = self.flags,
	userId = findParentBySelector(parent, '.modal').dataset.id;
	parent.querySelectorAll('.modal__ajax-content').forEach(function(content) {
		content.style.display = 'none'
	});
	if ((flags['content' + identificator] == true)) {
		let content = parent.querySelector('.modal__ajax-content[data-id="'+ identificator +'"]');
		content.style.display = 'block'
	}

	else {
		let addon = (isSpecific == true) ? ('/' + userId) : '',
		fullLink = window.base_url + link + addon,
		content = document.createElement('iframe');
		content.setAttribute('frameborder', 0);
		content.setAttribute('src', fullLink)
		...
		buttonGroups.forEach(function(group) {
			...
			childrensArray.forEach(function(children) {
				...
				listItem.onclick = function(event) {
					contentGenerator(listItem.dataset.href, findParentBySelector(listItem, '.modal__inner'), children.isSpecific, listItem.dataset.who)
				};
...
$(document).ready(function(event) {
	...
	(document.querySelector('.flexigrid')) && checkPage();
	if ($('.side-nav')) {
