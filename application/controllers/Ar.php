...
class Ar extends CI_Controller
{
	...
	$this->session->set_userdata('para_birim',"1");   
        ...
        if ($this->session->userdata('lng') != 'ar') {
        //    $this->dil();
...
/*			if(!empty($_SERVER['HTTP_REFERER'])){ 		
    ...
    {
        $cachetime = 0; // 10 saniyede bi cache yap
    ...
    public function anasayfa()
    {
        ...
        $this->cache_head("cache/anasayfa_ar.php");
        $this->load->model('user_model');
        $data["ayar"]         = $this->user_model->ayar();
        ...
        $data["canonical"]               = base_url()."ar";
        $data["alter"]       = base_url()."";
        $data["link_3"]     = base_url()."frs";		
        $this->load->view('ar/index', $data);

        $this->cache_foot("cache/anasayfa_ar.php");
    }

    public function contact()
    {$this->dil();
        $this->cache_head("cache/contact_ar.php");
        $this->load->model('user_model');
        $data["ayar"]      = $this->user_model->ayar();
        $data["proje_kat"] = $this->user_model->proje_kat(7);
        $data["haber_kat"] = $this->user_model->haber_kat(5);
        $data["linkler"]   = $this->user_model->linkler();
        $data["canonical"] = base_url()."ar/contact";
        $data["alter"]       = base_url()."contact";
		$data["link_3"]     = base_url()."frs/contact";
        $this->load->view('ar/iletisim', $data);
        $this->cache_foot("cache/contact_ar.php");
    }

    public function about()
    {$this->dil();
        $this->cache_head("cache/about_ar.php");
        $this->load->model('user_model');
        $data["ayar"]       = $this->user_model->ayar();
        $data["proje_kat"]  = $this->user_model->proje_kat(7);
        $data["haber_kat"]  = $this->user_model->haber_kat(5);
        ...
        $data["canonical"]  = base_url()."ar/about";
        $data["alter"]       = base_url()."about";
		$data["link_3"]     = base_url()."frs/about";
        $this->load->view('ar/hakkimizda', $data);
        $this->cache_foot("cache/about_ar.php");
    }
  ...
  public function search()
    {$this->dil();
        ...
        $this->load->model('user_model');
        $data["yer_adi"]="";
        if ($data["kat"] != "") {$data["kat_adi"] = $this->user_model->proje_kat_adi_id_ar($kat);}
        if ($data["tip"] != "") {$data["tip_adi"] = $this->user_model->proje_tip_ad_getir_ar($tip);}
        ...
        if ($data["yer"] != "") {$data["yer_adi"] = $this->user_model->unite_ad_getir_ar($yer);}
        ...
        $data["proje_kat_tum"]           = $this->user_model->proje_kat(100);
        $data["proje_yeri"]              = $this->user_model->unite_tip_getir_tum();
        $data["canonical"]               = base_url()."ar/search";
    ...
    {$this->dil();
        ...
        if ($data["oz"] != "") {
            ...
            $dongu = $say - 1;
            for ($i = 0; $i <= $dongu; $i++) {
                $data["oz_adi"][$i] = $this->user_model->proje_oz_ad_getir_ar($oz[$i]);
            }
        ...
        if ($data["yer"] != "") {
            ...
            $dongu = $say - 1;
            for ($i = 0; $i <= $dongu; $i++) {
                $data["unite_adi"][$i] = $this->user_model->unite_ad_getir_ar($yer[$i]);
            }
    ...
    {$this->dil();
        $this->cache_head("cache/kategori_" . $prop . "_ar.php");
        ...
        $data["canonical"]     = base_url()."/ar/" . $prop;
        $data["alter"]       = base_url()."". $prop;
		$data["link_3"]     = base_url()."frs/" . $prop;
        $this->load->view('ar/kategori', $data);
        $this->cache_foot("cache/kategori_" . $prop . "_ar.php");
    }
 ...
 $this->dil();
    ...
    {$this->dil();
        ...
        $this->cache_head("cache/kategori_tum_ar.php");
        $this->load->model('user_model');
        $data["ayar"]          = $this->user_model->ayar();
        $data["proje_kat"]     = $this->user_model->proje_kat(7);
        $data["haber_kat"]     = $this->user_model->haber_kat(5);
		...
		$data["page"]=ceil($data["say"]/21);
		$data["p"]=$p;	
        $data["canonical"]     = base_url()."ar/projects";
        $data["alter"]       = base_url()."projects";
		$data["link_3"]     = base_url()."frs/projects";
        $this->load->view('ar/kategori_tum', $data);
        $this->cache_foot("cache/kategori_tum_ar.php");
    ...
    {	$this->dil();
        ...
        $this->cache_head("cache/news_ar.php");
        $this->load->model('user_model');
        $data["ayar"]          = $this->user_model->ayar();
        $data["proje_kat"]     = $this->user_model->proje_kat(7);
        $data["haber_kat"]     = $this->user_model->haber_kat(5);
		...
		$data["p"]=$p;	
        $data["prop"]          = "";
        $data["canonical"]     = base_url()."ar/news";
        $data["alter"]       = base_url()."news";
		$data["link_3"]     = base_url()."frs/news";
        $this->load->view('ar/haber_kategori', $data);
        $this->cache_foot("cache/news_ar.php");
    }
    ...
    {$this->dil();
        ...
        $this->cache_head("cache/haber_kategori_" . $prop . "_ar.php");
        $this->load->model('user_model');
        $data["ayar"]          = $this->user_model->ayar();
        $data["proje_kat"]     = $this->user_model->proje_kat(7);
        $data["haber_kat"]     = $this->user_model->haber_kat(5);
        ...
        if ($prop == "buyer_guide") {
            ...
            $data["haber_kat_adi"] = $this->user_model->haber_kat_adi_ar($prop);}
      ...
      //  $data["kat_haber_manset_buyuk"]     = $this->user_model->kat_haber_manset_2_buyuk($hb_kt_id);
		...
		$data["link_3"]     = base_url()."frs/" . $prop;
        $this->load->view('ar/haber_kategori', $data);
        $this->cache_foot("cache/haber_kategori_" . $prop . "_ar.php");
    }

    public function proje($prop = null)
    {$this->dil();
        $this->cache_head("cache/proje_" . $prop . "_ar.php");
        $this->load->model('user_model');
        $data["ayar"]        = $this->user_model->ayar();
        $data["proje_kat"]   = $this->user_model->proje_kat(7);
        $data["haber_kat"]   = $this->user_model->haber_kat(5);
        ...
        if ($data["proje_detay"]): foreach ($data["proje_detay"] as $dizi):
                ...
                $data["kat"]     = $this->user_model->proje_kat_adi_id_ar($dizi["kategori"]);
        ...
        $n                          = 0;if ($data["unite_tip"]): foreach ($data["unite_tip"] as $dizi):
                $data["unite_tip_ad"][$n] = $this->user_model->unite_tip_ad_getir_ar($dizi["unite_tip_id"]);
                $n                        = $n + 1;endforeach;endif;
        $n                          = 0;if ($data["proje_tip"]): foreach ($data["proje_tip"] as $dizi):
                $data["proje_tip_ad"][$n] = $this->user_model->proje_tip_ad_getir_ar($dizi["proje_tip_id"]);
                $n                        = $n + 1;endforeach;endif;
        $data["canonical"]          = base_url()."ar/" . $prop;
        $data["alter"]       = base_url()."". $prop;
		$data["link_3"]     = base_url()."frs/" . $prop;
        ...
        $this->load->view('ar/proje', $data);
        $this->cache_foot("cache/proje_" . $prop . "_ar.php");
    }

    public function haber($prop = null)
    {$this->dil();

        $this->cache_head("cache/haber_" . $prop . "_ar.php");
        $this->load->model('user_model');
        $data["ayar"]         = $this->user_model->ayar();
        $data["proje_kat"]    = $this->user_model->proje_kat(7);
        $data["haber_kat"]    = $this->user_model->haber_kat(5);
        ...
        if ($data["haber_detay"]): foreach ($data["haber_detay"] as $dizi):
                $data["kat"]     = $this->user_model->haber_kat_adi_id_ar($dizi["kategori_id"]);
                $data["seo_adi"] = $this->user_model->haber_kat_seo_adi($dizi["kategori_id"]);
            endforeach;endif;
        $data["prop"]            = $prop;
        $data["haber_buyer_adi"] = $this->user_model->haber_buyer_adi_ar($prop);
        $data["canonical"]       = base_url()."ar/" . $prop;
        $data["alter"]       = base_url()."". $prop;
				$data["link_3"]     = base_url()."frs/" . $prop;
        $this->load->view('ar/haber_buyer', $data);
        $this->cache_foot("cache/haber_" . $prop . "_ar.php");
    ...
    {$this->dil();
        ...
        $this->cache_head("cache/buyer_" . $prop . "_ar.php");
        $this->load->model('user_model');
        $data["ayar"]            = $this->user_model->ayar();
		...
		$data["link_3"]     = base_url()."frs/" . $prop;
        $this->load->view('ar/haber_buyer', $data);
        $this->cache_foot("cache/buyer_" . $prop . "_ar.php");
    }
